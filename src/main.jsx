...
const getLongRate = (camera, key) => {
  ...
  const rates = normalizeLongRates(camera || {})
...
function Site({ data }) {
  ...
  return <div className={`siteApp theme-${theme}`}>
    ...
    <main id="top">
      ...
      <section className="section container" id="gia"><div className="sectionIntro oneCol"><div><div className="eyebrow">BẢNG GIÁ</div><h2>Thuê theo ngày,<br/><i>dài hơn thì rẻ hơn.</i></h2></div></div><div className="priceGrid">{plans.map((item,i)=>{const activeCameras=cameras.filter(c=>c.available!==false);const longKey = item.id==='1-day' ? '1-day' : item.id==='3-day' ? 3 : item.id==='7-day' ? 7 : null;const cheapestBy = key => activeCameras.length ? activeCameras.reduce((a,b)=>getLongRate(a,key)<=getLongRate(b,key)?a:b) : null;const cheapestHourly=activeCameras.length ? activeCameras.reduce((a,b)=>getHourlyRate(a,'1')<=getHourlyRate(b,'1')?a:b) : null;const cheapest=item.id==='hourly'?cheapestHourly:(longKey?cheapestBy(longKey):null);const displayPrice=cheapest ? (item.id==='hourly'?getHourlyRate(cheapest,'1'):getLongRate(cheapest,longKey)) : Number(item.price||0);return <article key={item.id} className={i===0?'priceCard featured':'priceCard'}><div className="priceTitle"><h3>{item.title}</h3>{item.badge&&<span>{item.badge}</span>}</div><div className="priceValue"><span className="pricePrefix">Chỉ từ</span>{formatVnd(displayPrice)}<small>/ máy</small></div><p>{item.note}</p>{cheapest&&<small className="priceFrom">Giá từ {cheapest.name}</small>}<button onClick={()=>{setChosen(null);setPlan(item.id);setModalOpen(true)}}>Chọn gói <ChevronRight size={15}/></button></article>})}</div><div className="hourlyRatesBlock"><div className="hourlyRatesIntro"><div><div className="eyebrow">GIÁ THEO GIỜ</div><h3>Mỗi máy một mức giá,<br/><i>chọn đúng thời lượng.</i></h3></div><p>Giá được tính riêng theo từng máy. Chọn 1 tiếng, 3 tiếng, 5 tiếng hoặc 1 buổi khi đặt máy.</p></div><div className="hourlyRatesTable"><div className="hourlyRatesHeader"><span>Máy ảnh</span><span>1 tiếng</span><span>3 tiếng</span><span>5 tiếng</span><span>1 buổi</span><span>1 ngày</span><span>3 ngày</span><span>1 tuần</span></div>{cameras.map(c=><div className="hourlyRatesRow" key={c.id}><strong>{c.name}</strong><span>{formatVnd(getHourlyRate(c,'1'))}</span><span>{formatVnd(getHourlyRate(c,'3'))}</span><span>{formatVnd(getHourlyRate(c,'5'))}</span><span>{formatVnd(getHourlyRate(c,'session'))}</span><span>{formatVnd(c.price)}</span><span>{formatVnd(getLongRate(c,3))}</span><span>{formatVnd(getLongRate(c,7))}</span></div>)}</div></div></section>

      <section className="policySection" id="cs"><div className="container section"><div className="sectionIntro"><div><div className="eyebrow">CHÍNH SÁCH THUÊ MÁY</div><h2>Rõ ràng trước,<br/><i>vui vẻ sau.</i></h2></div><p>Điều khoản cơ bản để cả Capy và bạn đều yên tâm.</p></div><div className="policyCards">{(policies||[]).slice(0,4).map(([num,title,text],i)=><div className="policyCard" key={num}><div className="policyIcon">{i===0?'✓':i===1?'↑':i===2?'◇':'!'}</div><div><h3>{title}</h3><p>{text}</p></div></div>)}</div>{policies?.[4]&&<div className="depositCard"><div><h3>{policies[4][1]}</h3><p>{policies[4][2]}</p></div><strong>{settings.phone}</strong></div>}</div></section>
...
function Admin({ data, setData }) {
  ...
  return <div className="adminShell"><aside className="adminSide"><div className="adminLogo"><span className="brandMark"><Camera size={18}/></span><b>Tiệm Ảnh Capy</b></div><div className="adminMenu">{[["cameras","Máy ảnh",Camera],["samples","Ảnh mẫu",ImageIcon],["plans","Bảng giá",Save],["policies","Chính sách",Settings],["orders","Đơn đặt máy",ClipboardList],["settings","Cài đặt",Settings]].map(([id,label,Icon])=><button key={id} className={tab===id?'active':''} onClick={()=>setTab(id)}><Icon size={17}/>{label}</button>)}</div><button className="logout" onClick={()=>{sessionStorage.removeItem('capy_admin');setAuthed(false)}}><LogOut size={16}/> Đăng xuất</button></aside>
    <main className="adminMain"><div className="adminTop"><div><span className="eyebrow">QUẢN TRỊ</span><h1>{tab==='cameras'?'Danh sách máy ảnh':tab==='samples'?'Ảnh mẫu':tab==='plans'?'Bảng giá':tab==='policies'?'Chính sách thuê':tab==='orders'?'Đơn đặt máy':'Cài đặt website'}</h1></div><a className="button ghost" href="/">Xem website ↗</a></div>
      ...
      {tab==='policies'&&<div className="policyAdminGrid">
        <div className="editPanel policyAdminIntro"><div className="eyebrow">CHỈNH TRỰC TIẾP</div><h2>Chính sách thuê máy</h2><p>Thay tiêu đề và nội dung các mục chính sách. Mục <b>05</b> cũng chính là phần <b>Cọc giữ máy</b> màu đen ở cuối khu vực chính sách trên website.</p></div>
        {(data.policies||[]).map((item,i)=><div className="editPanel policyAdminCard" key={item[0]||i}><div className="policyAdminNumber">{item[0]}</div><label>Tiêu đề<input value={item[1]||''} onChange={e=>{const x=[...(data.policies||[])];x[i]=[x[i][0],e.target.value,x[i][2]];update('policies',x)}}/></label><label>Nội dung<textarea rows={4} value={item[2]||''} onChange={e=>{const x=[...(data.policies||[])];x[i]=[x[i][0],x[i][1],e.target.value];update('policies',x)}}/></label></div>)}
      </div>}
...
function Root(){
  const [data,setDataState]=useState(()=>({cameras:loadCameras(),samples:normalizeSamples(load('capy_samples',defaultSamples)),plans:load('capy_plans',defaultPlans),policies:load('capy_policies',defaultPolicies),settings:load('capy_settings',defaultSettings),orders:load('capy_orders',[])}))
