...
import { dbConfigured, fetchData, signIn, signOut, isAdminSignedIn, adminEmail, upsertCamera, deleteCamera, upsertSample, deleteSample, upsertPlan, upsertSettings, insertOrder, uploadImage, migrateLocalStorage, checkCameraAvailable, getAvailableCameraIds } from './db'
import './styles.css'
...
function Site({ data }) {
  ...
  const openReserve = (camera = null) => { setChosen(camera); setPlan('1-day'); setCopied(false); setConflict(null); setAvailabilityChecked(false); setAvailableCameraIds([]); setCustomer({ name: '', phone: '', date: '', startTime: '09:00', endTime: '12:00', hours: 1, hourlyOption: '1' }); setModalOpen(true) }
  ...
  const checkAvailability = async () => {
    ...
    if (dbConfigured) {
      try {
        const ok = await checkCameraAvailable(chosen.id, customer.date, plan, customer.startTime, customer.hours)
        const ids = await getAvailableCameraIds(customer.date, plan, customer.startTime, customer.hours)
        setAvailableCameraIds(ids); setConflict(ok ? null : { camera: chosen.name, customer: { date: customer.date, startTime: customer.startTime, hours: customer.hours }, plan })
        setAvailabilityChecked(true)
        return ok
      } catch (e) { console.error(e); alert('Không thể kiểm tra lịch máy chủ. Vui lòng thử lại.'); return false }
    }
    const found = findConflict()
    setConflict(found); setAvailableCameraIds(cameras.filter(c=>c.available!==false && !findConflict(c.name)).map(c=>Number(c.id)))
    setAvailabilityChecked(true)
    return !found
  }
  const availableCameras = availabilityChecked && customer.date
    ? cameras.filter(c => c.available !== false && (dbConfigured ? availableCameraIds.includes(Number(c.id)) : !findConflict(c.name)))
    : []
  const messageText = () => {
  ...
  const submitOrder = async () => {
    ...
    const order = { cameraId: chosen.id, camera: chosen.name, plan, customer, createdAt: new Date().toISOString(), status: 'Mới' }
    try { await insertOrder(order) } catch (e) { alert('Không thể lưu yêu cầu. Vui lòng thử lại.'); return }
    setCopied(false)
  ...
  return <div className={`siteApp theme-${theme}`}>
    ...
    {modalOpen&&<div className="modalBackdrop" onClick={()=>setModalOpen(false)}><div className="modal" onClick={e=>e.stopPropagation()}><button className="modalClose" onClick={()=>setModalOpen(false)}><X size={18}/></button><div className="eyebrow">ĐẶT MÁY GIỮ CHỖ</div><h2>{chosen?.name||'Chọn máy tại Capy'}</h2><p>Chọn máy trực tiếp, thời gian thuê và thông tin liên hệ. Capy sẽ kiểm tra lịch trước khi xác nhận.</p><label>Chọn máy trực tiếp<select value={chosen?.id || ''} onChange={e=>{const cam=cameras.find(c=>String(c.id)===e.target.value);setChosen(cam||null);setConflict(null);setAvailabilityChecked(false);setAvailableCameraIds([])}}><option value="">— Chọn máy ảnh —</option>{cameras.map(c=><option key={c.id} value={c.id} disabled={c.available===false}>{c.name} · {c.brand}{c.available===false?' · đang thuê':''}</option>)}</select></label><div className="formGrid"><label>Họ tên<input value={customer.name} onChange={e=>setCustomer({...customer,name:e.target.value})} placeholder="Nguyễn Văn A"/></label><label>Số điện thoại<input value={customer.phone} onChange={e=>setCustomer({...customer,phone:e.target.value})} placeholder="09xxxxxxxx"/></label><label>Ngày nhận<input type="date" value={customer.date} onChange={e=>{setCustomer({...customer,date:e.target.value});setConflict(null);setAvailabilityChecked(false);setAvailableCameraIds([])}}/></label></div><div className="planPicker">{plans.map(item=>{const planPrice=item.id==='hourly'?(chosen?getHourlyRate(chosen, customer.hourlyOption || '1'):null):chosen?getCameraPlanPrice(chosen,item.id):null;return <button key={item.id} className={plan===item.id?'active':''} onClick={()=>{setPlan(item.id);setConflict(null);setAvailabilityChecked(false);setAvailableCameraIds([])}}>{item.title}{planPrice!=null?<small>{item.id==='hourly'?'Từ ':''}{formatVnd(planPrice)}</small>:null}</button>})}</div>{plan==='hourly'&&<><div className="hourlyOptionGrid">{hourlyOptions.map(opt=><button key={opt.key} type="button" className={customer.hourlyOption===opt.key?'active':''} onClick={()=>{setCustomer({...customer,hours:opt.hours,hourlyOption:opt.key});setConflict(null);setAvailabilityChecked(false);setAvailableCameraIds([])}}><b>{opt.label}</b><small>{chosen ? formatVnd(getHourlyRate(chosen,opt.key)) : 'Chọn máy'}</small></button>)}</div><div className="formGrid"><label>Giờ bắt đầu<input type="time" value={customer.startTime} onChange={e=>{setCustomer({...customer,startTime:e.target.value});setConflict(null);setAvailabilityChecked(false);setAvailableCameraIds([])}}/></label><div className="hourlySelectedPrice"><span>Giá thuê</span><strong>{chosen ? formatVnd(selectedHourlyPrice) : '—'}</strong><small>{hourlyOptions.find(x=>x.key===customer.hourlyOption)?.label || '1 tiếng'}</small></div></div></>}<button className="checkAvailabilityButton" onClick={checkAvailability}><Check size={16}/> Kiểm tra lịch máy</button><div className="customTimeBox"><div><strong>Cần thuê thời gian khác?</strong><span>Liên hệ Capy để được hỗ trợ thời gian thuê linh hoạt.</span></div><div className="customTimeActions"><a href={`tel:${settings.phone.replace(/\D/g,'')}`}><Phone size={15}/> Gọi điện</a><a href={settings.zalo || '#'} target="_blank" rel="noreferrer"><MessageCircle size={15}/> Zalo</a></div></div>{conflict&&<div className="conflictBox"><strong>⚠ Máy đã trùng lịch</strong><span>{chosen?.name} đã có lịch vào {conflict.customer?.date}{conflict.plan==='hourly' ? ` · ${conflict.customer?.startTime || ''} (${conflict.customer?.hours || 1} giờ)` : ' · cả ngày'}.</span><a className="hotlineButton" href={`tel:${settings.phone.replace(/\D/g,'')}`}><Phone size={16}/> Liên hệ Hotline {settings.phone}</a></div>}{availabilityChecked&&!conflict&&chosen&&<div className="availableBox"><Check size={16}/><div><strong>✓ Máy đang trống lịch</strong><span>{chosen.name} hiện còn trống cho thời gian bạn chọn.</span></div></div>}{availabilityChecked&&customer.date&&<div className="availabilityPanel"><div className="availabilityHead"><strong>Máy đang trống lịch</strong><span>{availableCameras.length} máy có thể đặt</span></div>{availableCameras.length ? <div className="availabilityList">{availableCameras.map(c=><button key={c.id} type="button" className={`availabilityItem ${chosen?.id===c.id?'selected':''}`} onClick={()=>{setChosen(c);setConflict(null);setAvailabilityChecked(true)}}><span className="availabilityDot"></span><span className="availabilityInfo"><b>{c.name}</b><small>{c.brand} · {c.type}</small></span><span className="availabilityPrice"><b>{hourlyOptions.find(x=>x.key===customer.hourlyOption)?.label || '1 tiếng'} {formatVnd(getHourlyRate(c, customer.hourlyOption || '1'))}</b><small>1 ngày {formatVnd(c.price)}</small></span>{chosen?.id===c.id&&<Check size={16}/>}</button>)}</div> : <div className="availabilityEmpty">Hiện không còn máy trống trong khung thời gian này.</div>}</div>}<div className="requestBox">{messageText()}</div><div className="modalActions"><button className="copyButton" onClick={copyRequest}>{copied?<Check size={16}/>:<Copy size={16}/>} {copied?'Đã sao chép':'Sao chép nội dung'}</button>{!conflict?<button className="button primary" onClick={submitOrder}>Lưu yêu cầu <ChevronRight size={15}/></button>:<a className="button hotlinePrimary" href={`tel:${settings.phone.replace(/\D/g,'')}`}><Phone size={15}/> Gọi Hotline</a>}</div></div></div>}
  </div>
}

function Admin({ data, setData, reloadData }) {
  const [authed, setAuthed] = useState(isAdminSignedIn())
  const [email, setEmail] = useState(localStorage.getItem('capy_admin_email') || '')
  const [password, setPassword] = useState('')
  const [tab, setTab] = useState('cameras')
  const [editing, setEditing] = useState(null)
  const [settingsDraft, setSettingsDraft] = useState(data.settings)
  const [busy, setBusy] = useState(false)
  React.useEffect(()=>setSettingsDraft(data.settings),[data.settings])

  const update = async (key, value) => {
    const next = {...data, [key]: value}; setData(next)
    try {
      if (dbConfigured) {
        if (key === 'cameras') { for (const c of value) await upsertCamera(c); for (const c of data.cameras.filter(c=>!value.some(n=>String(n.id)===String(c.id)))) await deleteCamera(c.id) }
        if (key === 'samples') { for (let i=0;i<value.length;i++) await upsertSample({...value[i], id:value[i].id || crypto.randomUUID(), sort_order:i}); for (const s of data.samples.filter(s=>!value.some(n=>String(n.id)===String(s.id)))) if(s.id) await deleteSample(s.id) }
        if (key === 'plans') for (let i=0;i<value.length;i++) await upsertPlan(value[i],i)
        if (key === 'settings') await upsertSettings(value)
      } else save(`capy_${key}`, value)
    } catch (e) { alert('Không thể đồng bộ dữ liệu lên máy chủ. Vui lòng kiểm tra lại cấu hình Database.'); console.error(e) }
  }
  const login = async e => {
    e.preventDefault();
    if (!dbConfigured) { alert('Website chưa kết nối Database. Hãy cấu hình Supabase trước.'); return }
    setBusy(true); try { await signIn(email,password); setAuthed(true); await reloadData(true) } catch (err) { alert(err.message || 'Đăng nhập thất bại.') } finally { setBusy(false) }
  }
  if(!authed) return <div className="adminLogin"><form onSubmit={login} className="loginCard"><div className="brandMark"><Camera size={19}/></div><h1>Quản trị Tiệm Ảnh Capy</h1><p>Đăng nhập bằng tài khoản quản trị để quản lý dữ liệu dùng chung trên PC và điện thoại.</p><input autoFocus type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="Email quản trị"/><input type="password" value={password} onChange={e=>setPassword(e.target.value)} placeholder="Mật khẩu"/><button className="button primary" disabled={busy}>{busy?'Đang đăng nhập…':'Đăng nhập'}</button><a href="/">← Về trang chủ</a></form></div>

  const saveCamera = async () => { const normalized = {...editing, hourlyRates: normalizeHourlyRates(editing), longRates: normalizeLongRates(editing)}; const list = editing.id ? data.cameras.map(c=>c.id===editing.id?normalized:c) : [...data.cameras,{...normalized,id:Date.now(),available:true}]; await update('cameras',list); setEditing(null) }
  const removeCamera = id => { if(confirm('Xóa máy ảnh này?')) update('cameras',data.cameras.filter(c=>c.id!==id)) }
  const addSample = async (e) => { const file=e.target.files?.[0]; if(!file)return; if(file.size>4*1024*1024){alert('Ảnh mẫu tối đa 4MB.');e.target.value='';return} try { const src=dbConfigured ? await uploadImage(file,'samples') : await new Promise(resolve=>{const r=new FileReader();r.onload=()=>resolve(String(r.result));r.readAsDataURL(file)}); const description=prompt('Nhập mô tả cho ảnh mẫu:') || ''; await update('samples',[...data.samples,{id:crypto.randomUUID(),src,description,sort_order:data.samples.length}]) } catch(err){alert('Không thể tải ảnh lên máy chủ.')} e.target.value='' }
  ...
  const saveSettings = async () => { const next={...data,settings:settingsDraft}; setData(next); if(dbConfigured){try{await upsertSettings(settingsDraft); alert('Đã lưu cài đặt và đồng bộ.')}catch(e){alert('Không thể lưu cài đặt.')}} else {save('capy_settings',settingsDraft); alert('Đã lưu cài đặt.')} }
  const handleBannerUpload = e => {
    const file = e.target.files?.[0]
    if (!file) return
    if (!file.type.startsWith('image/')) { alert('Vui lòng chọn file ảnh.'); return }
    if (file.size > 4 * 1024 * 1024) { alert('Ảnh banner tối đa 4MB để website tải nhanh.'); return }
    if (dbConfigured) { uploadImage(file,'banners').then(src=>setSettingsDraft({...settingsDraft,heroImage:src})).catch(()=>alert('Không thể tải banner lên máy chủ.')) }
    else { const reader = new FileReader(); reader.onload = () => setSettingsDraft({...settingsDraft, heroImage: String(reader.result)}); reader.readAsDataURL(file) }
  }

  return <div className="adminShell"><aside className="adminSide"><div className="adminLogo"><span className="brandMark"><Camera size={18}/></span><b>Tiệm Ảnh Capy</b></div><div className="adminMenu">{[["cameras","Máy ảnh",Camera],["samples","Ảnh mẫu",ImageIcon],["plans","Bảng giá",Save],["orders","Đơn đặt máy",ClipboardList],["settings","Cài đặt",Settings]].map(([id,label,Icon])=><button key={id} className={tab===id?'active':''} onClick={()=>setTab(id)}><Icon size={17}/>{label}</button>)}</div><button className="logout" onClick={async()=>{await signOut();setAuthed(false)}}><LogOut size={16}/> Đăng xuất</button></aside>
