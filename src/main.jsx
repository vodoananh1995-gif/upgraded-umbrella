...
function Site({ data }) {
  ...
  return <div className={`siteApp theme-${theme}`}>
    ...
    <main id="top">
      ...
      <section className="section container" id="gia"><div className="sectionIntro oneCol"><div><div className="eyebrow">BẢNG GIÁ</div><h2>Thuê theo ngày,<br/><i>dài hơn thì rẻ hơn.</i></h2></div></div><div className="priceGrid">{plans.map((item,i)=>{const activeCameras=cameras.filter(c=>c.available!==false);const cheapest=activeCameras.length?activeCameras.reduce((a,b)=>Number(a.price||Infinity)<=Number(b.price||Infinity)?a:b):null;const oneDayPlan=Number(plans.find(p=>p.id==='1-day')?.price||120000);const cheapestDaily=cheapest?Number(cheapest.price||oneDayPlan):oneDayPlan;const configured=Number(item.price||0);const displayPrice=item.id==='hourly'?(cheapest?getHourlyRate(cheapest,'1'):configured):item.id==='1-day'?cheapestDaily:cheapest?Math.round((cheapestDaily*(configured/oneDayPlan))/10000)*10000:configured;return <article key={item.id} className={i===0?'priceCard featured':'priceCard'}><div className="priceTitle"><h3>{item.title}</h3>{item.badge&&<span>{item.badge}</span>}</div><div className="priceValue"><span className="pricePrefix">Chỉ từ</span>{formatVnd(displayPrice)}<small>/ máy</small></div><p>{item.note}</p>{cheapest&&<small className="priceFrom">Giá từ {cheapest.name}</small>}<button onClick={()=>{setChosen(null);setPlan(item.id);setModalOpen(true)}}>Chọn gói <ChevronRight size={15}/></button></article>})}</div><div className="hourlyRatesBlock"><div className="hourlyRatesIntro"><div><div className="eyebrow">GIÁ THEO GIỜ</div><h3>Mỗi máy một mức giá,<br/><i>chọn đúng thời lượng.</i></h3></div><p>Giá được tính riêng theo từng máy. Chọn 1 tiếng, 3 tiếng, 5 tiếng hoặc 1 buổi khi đặt máy.</p></div><div className="hourlyRatesTable"><div className="hourlyRatesHeader"><span>Máy ảnh</span><span>1 tiếng</span><span>3 tiếng</span><span>5 tiếng</span><span>1 buổi</span><span>1 ngày</span><span>3 ngày</span><span>1 tuần</span></div>{cameras.map(c=><div className="hourlyRatesRow" key={c.id}><strong>{c.name}</strong><span>{formatVnd(getHourlyRate(c,'1'))}</span><span>{formatVnd(getHourlyRate(c,'3'))}</span><span>{formatVnd(getHourlyRate(c,'5'))}</span><span>{formatVnd(getHourlyRate(c,'session'))}</span><span>{formatVnd(c.price)}</span><span>{formatVnd(getCameraPlanPrice(c,'3-day'))}</span><span>{formatVnd(getCameraPlanPrice(c,'7-day'))}</span></div>)}</div></div></section>
