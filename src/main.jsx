...
const defaultCameras = [
  { id: 1, name: 'Canon Sure Shot', brand: 'Canon', type: 'Máy phim', price: 120000, lens: '38mm f/2.8', desc: 'Point & shoot nhỏ gọn, màu ảnh hoài niệm.', image: 'https://images.unsplash.com/photo-1516035069371-29a1b244cc32?auto=format&fit=crop&w=1200&q=85', available: true },
  { id: 2, name: 'Fujifilm X-A5', brand: 'Fujifilm', type: 'Máy digital', price: 120000, lens: '15–45mm', desc: 'Nhẹ, dễ dùng, hợp đi chơi và chụp đời thường.', image: 'https://images.unsplash.com/photo-1502920917128-1aa500764cbd?auto=format&fit=crop&w=1200&q=85', available: true },
  { id: 3, name: 'Olympus Mju', brand: 'Olympus', type: 'Máy phim', price: 150000, lens: '35mm f/2.8', desc: 'Nhỏ gọn, flash tiện dụng, màu phim rất dễ thương.', image: 'https://images.unsplash.com/photo-1452780212940-6f5c0d14d848?auto=format&fit=crop&w=1200&q=85', available: true },
  { id: 4, name: 'Canon AE-1', brand: 'Canon', type: 'Máy phim', price: 180000, lens: '50mm f/1.8', desc: 'Máy film cơ kinh điển cho người thích trải nghiệm.', image: 'https://images.unsplash.com/photo-1606986628253-4f56d7a7e83a?auto=format&fit=crop&w=1200&q=85', available: true },
  { id: 5, name: 'Fujifilm X-T20', brand: 'Fujifilm', type: 'Máy digital', price: 180000, lens: '18–55mm', desc: 'Chất ảnh đẹp, thân máy nhỏ, hợp chụp phố.', image: 'https://images.unsplash.com/photo-1452780212940-6f5c0d14d848?auto=format&fit=crop&w=1200&q=85', available: true },
  { id: 6, name: 'Olympus Trip 35', brand: 'Olympus', type: 'Máy phim', price: 130000, lens: '40mm f/2.8', desc: 'Một chiếc máy cổ điển rất dễ làm quen.', image: 'https://images.unsplash.com/photo-1526170375885-4d8ecf77b99f?auto=format&fit=crop&w=1200&q=85', available: true },
]
...
const defaultPlans = [
  { id: 'hourly', title: 'Theo giờ', price: 60000, note: 'Linh hoạt cho buổi chụp ngắn. Tính theo giờ.', badge: 'Linh hoạt' },
...
function Site({ data }) {
  ...
  const [customer, setCustomer] = useState({ name: '', phone: '', date: '', startTime: '09:00', endTime: '12:00', hours: 1 })
  ...
  const openReserve = (camera = null) => { setChosen(camera); setPlan('1-day'); setCopied(false); setConflict(null); setCustomer({ name: '', phone: '', date: '', startTime: '09:00', endTime: '12:00', hours: 1 }); setModalOpen(true) }
  ...
  const messageText = () => {
    ...
    const timeText = plan === 'hourly' ? `${customer.startTime || '--:--'} - ${customer.startTime ? `${String(Math.floor((timeToMinutes(customer.startTime) + Number(customer.hours || 1)*60)/60)%24).padStart(2,'0')}:${String((timeToMinutes(customer.startTime) + Number(customer.hours || 1)*60)%60).padStart(2,'0')}` : '--:--'} (${customer.hours || 1} giờ)` : selectedPlan?.title || '1 ngày'
    return `Xin chào Capy, mình muốn thuê ${machine} trong ${timeText}. Tên: ${customer.name || 'chưa nhập'}. SĐT: ${customer.phone || 'chưa nhập'}. Ngày nhận: ${customer.date || 'chưa chọn'}. Nhờ Capy kiểm tra tình trạng máy và báo giúp mình tiền cọc.`
  }
  ...
  return <div className={`siteApp theme-${theme}`}>
    ...
    <main id="top">
      <section className="hero container"><div className="heroCopy"><div className="photoBadge"><Camera size={14}/> FILM / DIGITAL / EVERYDAY</div><div className="eyebrow">✦ CAMERA RENTAL · BÀU TRÓ</div><h1>Cầm máy lên,<br/><i>còn lại để Capy lo.</i></h1><p>Chọn chiếc máy hợp gu, xem thông tin rõ ràng và giữ chỗ trong vài phút. Mỗi máy đều được test kỹ trước khi giao.</p><div className="heroBenefits"><div><span className="benefitIcon"><Check size={15}/></span><div><strong>Máy chất lượng</strong><small>Test kỹ trước khi giao</small></div></div><div><span className="benefitIcon"><Camera size={15}/></span><div><strong>Thủ tục đơn giản</strong><small>Đặt nhanh qua Zalo</small></div></div><div><span className="benefitIcon"><Phone size={15}/></span><div><strong>Hỗ trợ tận tình</strong><small>Tư vấn chọn máy</small></div></div></div><div className="heroActions"><a className="button primary" href="#may">Xem máy cho thuê <ChevronRight size={17}/></a><button className="button ghost" onClick={() => openReserve()}>Liên hệ đặt cọc <MessageCircle size={16}/></button></div><div className="heroMeta"><span className="metaAvatars"><i></i><i></i><i></i><b>+6</b></span><span>{cameras.filter(c=>c.available !== false).length} máy sẵn sàng</span><span>•</span><span>Hỗ trợ nhanh qua Zalo</span></div><div className="heroMotto"><span></span> SHOOT MORE, WORRY LESS</div></div><div className="heroVisual"><div className="heroFloatingNote"><b>● FILM VIBES</b><span>DIGITAL FREEDOM</span><span>SAME PASSION</span></div><img src={settings.heroImage} alt="Tiệm Ảnh Capy"/><div className="heroSticker"><div><strong>Ảnh thật · Máy thật</strong><span>Test trước khi giao</span></div><div className="heroSpecs"><b>f/1.8</b><span>1/125</span><span>ISO 400</span></div></div><div className="verticalTag">CAPY / FILM & DIGITAL</div><div className="heroStamp"><Camera size={20}/><span>CAMERA<br/>READY</span></div></div></section>
...
function Admin({ data, setData }) {
  ...
  const saveCamera = () => { const list = editing.id ? data.cameras.map(c=>c.id===editing.id?editing:c) : [...data.cameras,{...editing,id:Date.now(),available:true}]; update('cameras',list); setEditing(null) }
  ...
  return <div className="adminShell"><aside className="adminSide"><div className="adminLogo"><span className="brandMark"><Camera size={18}/></span><b>Tiệm Ảnh Capy</b></div><div className="adminMenu">{[["cameras","Máy ảnh",Camera],["samples","Ảnh mẫu",ImageIcon],["plans","Bảng giá",Save],["orders","Đơn đặt máy",ClipboardList],["settings","Cài đặt",Settings]].map(([id,label,Icon])=><button key={id} className={tab===id?'active':''} onClick={()=>setTab(id)}><Icon size={17}/>{label}</button>)}</div><button className="logout" onClick={()=>{sessionStorage.removeItem('capy_admin');setAuthed(false)}}><LogOut size={16}/> Đăng xuất</button></aside>
    <main className="adminMain"><div className="adminTop"><div><span className="eyebrow">QUẢN TRỊ</span><h1>{tab==='cameras'?'Danh sách máy ảnh':tab==='samples'?'Ảnh mẫu':tab==='plans'?'Bảng giá':tab==='orders'?'Đơn đặt máy':'Cài đặt website'}</h1></div><a className="button ghost" href="/">Xem website ↗</a></div>
      {tab==='cameras'&&<><div className="adminToolbar"><p>{data.cameras.length} máy ảnh</p><button className="button primary" onClick={()=>setEditing({name:'',brand:'Canon',type:'Máy phim',price:120000,lens:'',desc:'',image:'',available:true})}><Plus size={16}/> Thêm máy ảnh</button></div><div className="adminTable">{data.cameras.map(c=><div className="adminRow" key={c.id}><img src={c.image} alt=""/><div><b>{c.name}</b><span>{c.brand} · {c.type}</span></div><strong>{formatVnd(c.price)}</strong><em className={c.available===false?'off':''}>{c.available===false?'Đang thuê':'Có sẵn'}</em><div className="rowActions"><button onClick={()=>setEditing({...c})}><Pencil size={15}/></button><button onClick={()=>removeCamera(c.id)}><Trash2 size={15}/></button></div></div>)}</div></>}
